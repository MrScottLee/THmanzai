...
function da_digit(parm, place, out) {
    var limit = Math.pow(10, place-1);
    var temp = {};
    for (var j = 0; j < db_rank.length; j++) {
        if (db_num[j] >= limit) {
            temp[j] = db_num[j].split("");
            if (parm == temp[j][temp[j].length - place]) {
                da_base(out, j);
            }
        }
    }
...
function da_qty(parm, freq, out) {
    var temp = {};
    for (var j = 0; j < db_rank.length; j++) {
        temp[j] = db_num[j].split(parm);
...
function num_adv(out_num) {
    ...
    da_sgl_appr(parm_num, db_num, 50, out_num["appr"])
    var up_num = Number(parm_num) + 50,
        low_num = Number(parm_num) - 50;
    var desc_num_a = "参赛编号在" + low_num + "至" + up_num + "之间";
    da_claim(out_num["appr"], desc_num_a);
    
    var digit = String(parm_num).split("");
